...
func open():
	if is_instance_valid(t):
		t.stop()
	t=create_tween()
	t.set_parallel().tween_property(door_l, 'position:x', open_pos_l, 1)
	t.set_parallel().tween_property(door_r, 'position:x', open_pos_r, 1)
	t.play()
...
func close():
	if is_instance_valid(t):
		t.stop()
	t=create_tween()
	t.set_parallel().tween_property(door_l, 'position:x', 0, 1)
	t.set_parallel().tween_property(door_r, 'position:x', 0, 1)
	t.play()
